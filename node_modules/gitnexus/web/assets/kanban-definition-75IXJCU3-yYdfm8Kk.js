...
`+e);let s=o.db.getData(),p=r();p.htmlLabels=!1;let m=f(t);for(let e of s.nodes)e.domId=`${t}-${e.id}`;let h=m.append(`g`);h.attr(`class`,`sections`);let g=m.append(`g`);g.attr(`class`,`items`);let _=s.nodes.filter(e=>e.isGroup),v=0,y=[],b=25;for(let e of _){let t=p?.kanban?.sectionWidth||200;v+=1,e.x=t*v+(v-1)*10/2,e.width=t,e.y=0,e.height=t*3,e.rx=5,e.ry=5,e.cssClasses=e.cssClasses+` section-`+v;let n=await l(h,e);b=Math.max(b,n?.labelBBox?.height),y.push(n)}let x=0;for(let e of _){let t=y[x];x+=1;let n=p?.kanban?.sectionWidth||200,r=-n*3/2+b,i=r,a=s.nodes.filter(t=>t.parentId===e.id);for(let t of a){if(t.isGroup)throw Error(`Groups within groups are not allowed in Kanban diagrams`);t.x=e.x,t.width=n-1.5*10;let r=(await d(g,t,{config:p})).node().getBBox();t.y=i+r.height/2,await u(t),i=t.y+r.height/2+10/2}let o=t.cluster.select(`rect`),c=Math.max(i-r+30,50)+(b-25);o.attr(`height`,c)}a(void 0,m,p.mindmap?.padding??i.kanban.padding,p.mindmap?.useMaxWidth??i.kanban.useMaxWidth)},`draw`)},A=s(e=>{let r=``;for(let r=0;r<e.THEME_COLOR_LIMIT;r++)e[`lineColor`+r]=e[`lineColor`+r]||e[`cScaleInv`+r],o(e[`lineColor`+r])?e[`lineColor`+r]=t(e[`lineColor`+r],20):e[`lineColor`+r]=n(e[`lineColor`+r],20);let i=s((r,i)=>e.darkMode?n(r,i):t(r,i),`adjuster`);for(let t=0;t<e.THEME_COLOR_LIMIT;t++){let n=``+(17-3*t);r+=`
  ...
  .kanban-ticket-link {
    ...
    `}return r},`genSections`),j={db:O,renderer:k,parser:_,styles:s(e=>`
  .edge {
    stroke-width: 3;
  }
  ${A(e)}
