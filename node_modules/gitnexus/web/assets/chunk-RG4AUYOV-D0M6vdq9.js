...
.edgeLabel[data-look="neo"] {
  ...
  ${y()}
`,`getStyles`),M=f((e,t=`TB`)=>{if(!e.doc)return t;let n=t;for(let t of e.doc)t.stmt===`dir`&&(n=t.value);return n},`getDir`),N={getClasses:f(function(e,t){return t.db.getClasses()},`getClasses`),draw:f(async function(e,t,n,r){m.info(`REF0:`),m.info(`Drawing class diagram (v3)`,t);let{securityLevel:i,state:a,layout:o}=c();r.db.setDiagramId(t);let s=r.db.getData(),l=x(t,i);s.type=r.type,s.layoutAlgorithm=v(o),s.nodeSpacing=a?.nodeSpacing||50,s.rankSpacing=a?.rankSpacing||50,s.markers=[`aggregation`,`extension`,`composition`,`dependency`,`lollipop`],s.diagramId=t,await _(s,l),g.insertTitle(l,`classDiagramTitleText`,a?.titleTopMargin??25,r.db.getDiagramTitle()),S(l,8,`classDiagram`,a?.useMaxWidth??!0)},`draw`),getDir:M};export{A as i,j as n,N as r,w as t};